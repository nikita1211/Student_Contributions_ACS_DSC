    ...
    public String getGoodWordStartingWith(String prefix, int whoEndsFirst) {
        ...
        if(prefix == ""){
            ...
            while(true){
                ...
                else oddwords.add(wordFromWordsList);
            }
            while(true){
                downIndex--;
                if(downIndex < 0){
        ...
        if(shortListedWord.size() == 0){
            ...
            Collections.sort(shortListedWord,new MyComparator() );
            int selectedWordIndex =0;// randomWordIndex.nextInt(shortListedWord.size());
            selected = shortListedWord.get(selectedWordIndex);
            if(selected.equals(prefix)){
                return "sameAsPrefix";
            }
            selectedWordIndex=shortListedWord.size()-1;
            int oddwordindex=oddwords.size()-1;
            selected=shortListedWord.get(selectedWordIndex);
            while(oddwordindex!=-1)
            {
                selected=shortListedWord.get(selectedWordIndex);
                if(selected.startsWith(oddwords.get(oddwordindex))){
                    if(selectedWordIndex==0) break;
                    selectedWordIndex--;
                    oddwordindex=oddwords.size()-1;
                }
                else
                oddwordindex--;
            }
